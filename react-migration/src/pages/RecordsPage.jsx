...
export function RecordsPage({ isAdmin, period, setPeriod, brigade, setBrigade, squad, setSquad, unit, setUnit, search, setSearch, resetPages, pagedRec, filtered, recPage, setRecPage, setEditTarget }) {
  const [viewMode, setViewMode] = useState('converted'); // 'converted' | 'raw'
  ...
  return (
    <section>
      {!isAdmin && (
        <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 6, padding: '10px 12px', marginBottom: 12, fontSize: 13, color: '#1e3a8a' }}>
      ...
      <div style={{ display: 'flex', gap: 6, margin: '12px 0 8px', alignItems: 'center' }}>
        <button style={btnStyle(viewMode === 'converted')} onClick={() => setViewMode('converted')}>
          換算得分
        </button>
                ...
                {pagedRec.map(r => {
                  ...
                  return (
                    <tr key={r.id} style={!s.pass ? { background: '#fff5f5' } : {}}>
                      ...
                      {isAdmin && (
                        <td>
                          <button onClick={() => setEditTarget(r)} style={{ border: '1px solid #94a3b8', background: '#f8fafc', borderRadius: 4, padding: '3px 8px', cursor: 'pointer', fontSize: 12 }}>
                            編輯
                          </button>
                        </td>
                      )}
              ...
              <tbody>
                {pagedRec.map(r => (
                  <tr key={r.id}>
                    ...
                    {RAW_COLS.map(c => (
                      <td key={c.key} style={{ color: r[c.key] == null ? '#94a3b8' : undefined }}>
                        {r[c.key] ?? '—'}
                      </td>
                    ))}
                    {isAdmin && (
